...
//     <div className="">
//     {/* Category Tabs */}
//     <div className="border-b border-gray-400 flex space-x-6 mb-4 xs:justify-center lg:justify-start">
//       {profileCategories.map((category) => (
//         <button
//           key={category}
...
const CreatorProfile = ({ creatorDetails, projects }) => { // <--- Added projects here
  const [selectedProfileCategory, setSelectedProfileCategory] = useState("PORTFOLIO"); // Default to PORTFOLIO
  const profileCategories = ["PORTFOLIO", "SERVICES", "ASSETS"];

  // Extract the creatorId and creatorName from creatorDetails
  const creatorId = creatorDetails?.creatorId;
  const creatorName = creatorDetails?.CreatorName;

  // Function to render the appropriate component based on selectedProfileCategory
  const renderProfileComponent = () => {
    switch (selectedProfileCategory) {
      case "PORTFOLIO":
        return <ProfilePortfolio creatorId={creatorId} creatorName={creatorName} projects={projects} />; // Pass projects
      case "SERVICES":
        return <ProfileServices creatorId={creatorId} projects={projects} />; // Pass projects (if services also need projects)
      case "ASSETS":
        return <ProfileAssets creatorId={creatorId} creatorName={creatorName} />; // Projects usually aren't needed for assets, but you can pass if required
      default:
        return null;
    }
  };
